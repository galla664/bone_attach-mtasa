...
local isElementStreamedIn 	= isElementStreamedIn;
...
function putAttachedElementsOnBones()
	local iPlayerDimension = getElementDimension( localPlayer );
	local iPlayerInterior = getElementInterior( localPlayer );
	...
	for element,ped in pairs(attached_ped) do
		...
		elseif isElementStreamedIn(ped) then
			if getElementDimension( ped ) == iPlayerDimension and getElementInterior( ped ) == iPlayerInterior then
			...
			if ped == localPlayer then
				local ped_weapon = getPedWeapon( ped )
				if getPedTask( ped, "secondary", 0 ) == "TASK_SIMPLE_USE_GUN" and ( ped_weapon == 34 or ped_weapon == 43 ) then
